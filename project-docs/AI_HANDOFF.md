...
- Business owners choose an employee, assign Manager or Staff role, and configure per-location permissions at `/businesses/[businessSlug]/admin/employees`. Migration `030_unified_employee_permissions.sql` consolidates the temporary permission tables into `employee_permissions`.
- The same Employee Permissions page can invite a new employee through Supabase Auth, save profile/contact details, and create the required `business_users`, `location_users`, and `employee_permissions` records.

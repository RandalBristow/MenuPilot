...
- `/businesses/[businessSlug]/specials` tenant-scoped public Specials & Deals route. It uses the same active public specials data as the menu preview, shows passive specials with "Applied automatically at checkout", and opens DealBuilder or MixAndMatchBuilder for buildable deal types.
- Product configurator with reusable variant groups, modifier groups, included credits, multiplier-aware pricing, hydrated product default modifier selections, default cart integration, and return-configured-result mode for DealBuilder child products. Valid product defaults appear selected in customer builders and can satisfy required groups. Deal child configuration can receive component pricing context so fixed-price and included/free deal components show component price plus extras in the sticky footer instead of the product's normal base/variant price. In fixed/included deal component mode, normal base/variant prices are hidden in the selector and only upcharges/extras remain visible.
- Business-level pricing settings are stored in `business_pricing_settings` and managed from Platform Admin business detail and the tenant admin shell. Defaults are pizza half pricing on, half included-slot counting on, `floor_to_cent` rounding, no sales tax, no service fee, and tips disabled.
- Cart provider, summary bar, sheet UI, and localStorage persistence.
- `/checkout` legacy pickup checkout for Pronto Demo/main-street and `/businesses/[businessSlug]/checkout` tenant-scoped checkout using the business default location. Checkout validates business/location orderability, rejects cross-tenant carts, validates/reprices cart contents server-side, applies passive Specials, calculates business-level service fee/tax/tip totals server-side, writes unpaid orders to Supabase, and links successful tenant orders to `/businesses/[businessSlug]/orders/[orderNumber]`.
- `/businesses/[businessSlug]/orders/[orderNumber]` customer-facing order status route. It loads by business slug and order number, shows safe order/status/summary details, nested deal/Mix children, modifiers, discounts, service fee, tax, tip, restaurant info, and current saved staff status on refresh. It does not expose internal IDs, payment data, customer email, or customer phone.
- `/staff/orders` legacy staff queue for Pronto Demo/main-street and `/businesses/[businessSlug]/locations/[locationSlug]/orders` tenant/location-scoped staff queue. Staff reads filter by business/location IDs, status updates resolve slugs server-side before verifying order ownership, and order cards show service fee/tax/tip rows when present.
- `/admin` admin hub.
- `/businesses/[businessSlug]/admin/media` tenant Media Library for image upload/import metadata backed by `media_assets`.
- `/businesses/[businessSlug]/admin/catalog/products` tenant product management hub and product/category/subcategory/product form flows.
...
- Quick 86 / temporarily sold out foundation and public runtime enforcement exist. `product_operational_availability` and `modifier_option_operational_availability` store temporary operational overrides separately from permanent `is_enabled`. `features/availability/utils/resolve-operational-availability.ts` resolves permanent enabled state plus active business/location overrides. Tenant Product Admin and Modifier Library rows can toggle business-wide product and Modifier Option 86 state. Public menu reads hide active business-wide sold-out products; ProductConfigurator loading blocks sold-out products and filters sold-out Modifier Options; DealBuilder/Mix child product choices exclude active sold-out products; checkout reloads availability server-side and rejects stale sold-out products/options. Staff/location-specific controls, location-scoped runtime enforcement, and realtime customer refresh are not wired yet.
...
- Tenant-aware business admin shell exists at `/businesses/[businessSlug]/admin`. Platform Admin business detail links to it with "Open Business Admin". The landing page is the top-level business dashboard for Product Catalog, Specials & Promotions, Media Library, Orders & Operations, and Storefront Preview. `/businesses/[businessSlug]/admin/catalog` contains only products, categories/subcategories, reusable Variant Groups, and the reusable Modifier Library.
- Tenant-scoped Product Admin routes live only under `/businesses/[businessSlug]/admin/catalog/products...`. They use the selected business for product/category/subcategory/variant/modifier-assignment reads and writes.
- Tenant-scoped Modifier Library routes live only under `/businesses/[businessSlug]/admin/catalog/modifiers...`. They validate selected-business ownership for reusable modifier reads and writes.
- Tenant-scoped Media lives at `/businesses/[businessSlug]/admin/media` and uses the selected business for reads, uploads, imports, metadata, and storage paths.
- Location manager routes live under `/businesses/[businessSlug]/locations/[locationSlug]/manager`. The dashboard links to manager orders, product availability, and separate modifier availability. Manager catalog editing is intentionally deferred until permissions are defined.
- Business owners manage employee details, one Manager or Staff role, multiple assigned locations, and permissions shared across those locations at `/businesses/[businessSlug]/admin/employees`. Migration `030_unified_employee_permissions.sql` consolidates the temporary permission tables into `employee_permissions`.
- The Manage Employees page can invite a new employee through Supabase Auth, save profile/contact details, and create the required `business_users`, `location_users`, and `employee_permissions` records for every selected location.
- Specials MVP schema support, a pure passive resolver, checkout integration, tenant-scoped Specials Admin UI, staff order discount display, public menu specials display, a tenant-scoped public Specials & Deals page, orderable-deal component schema/types, Specials Admin component editing, tenant-scoped Mix & Match admin editing, pure orderable deal and Mix & Match validation/pricing helpers, public DealBuilder runtime, public MixAndMatchBuilder runtime, nested deal cart type/display support, checkout/order snapshots for orderable and Mix & Match deals, and staff nested deal display exist. `specials`, `special_products`, `special_menu_groups`, and `special_availability_windows` are business-scoped setup tables for passive line discounts, fixed-price line specials, cart discounts, eligibility, and recurring day/time availability. `special_components` and `special_component_products` define orderable deal component slots and exact selectable products. `special_component_product_variant_options` optionally restricts a selected component product to specific reusable variant options; no rows means all enabled variants for that product are allowed. `special_component_modifier_group_overrides` optionally overrides the included count for an assigned Modifier Group on a selected component product only inside that deal context; no row means use the product's normal included rule. In Specials Admin, `orderable_deal` uses `fixed_price`, `discount_value` is the deal base price, and component saves manage labels/descriptions, quantity rules, exact allowed product choices, optional variant restrictions, and optional modifier included-count overrides. `applySpecialsToPricedCart` consumes server-validated priced normal cart lines and chooses the best single eligible passive special. `validateAndPriceOrderableDeal` validates already-priced proposed deal selections and prices deal base plus child extras. `validateAndPriceMixAndMatchDeal` validates already-priced proposed Mix & Match selections and prices selected quantity times unit price plus child extras. DealBuilder and MixAndMatchBuilder load active public deals, use image-backed product cards with default add/customize paths, pass variant restrictions and Modifier Group included-count overrides into ProductConfigurator return mode, validate nested selections, and add one parent deal item to cart. DealBuilder's Add to Deal path uses real product defaults only; it opens ProductConfigurator instead of quick-adding when required defaults are missing or an included Modifier Group still has unused included selections. Checkout reloads active deal configuration, validates child products, variant restrictions, and deal-context modifier included-count overrides server-side, recalculates child extras, rejects stale deal totals, and inserts parent/child `order_items` rows. Staff orders display subtotal/discount/total, applied passive discount snapshots, and nested deal children. Public menus show active Current Specials, eligible line/fixed-price product badges, a View all specials link, Build Deal actions, and Build Mix & Match actions; `/businesses/[businessSlug]/specials` shows the full active public Specials & Deals list with type badges and CTAs. Checkout remains authoritative. Passive specials do not apply to deal items in MVP; rich cart preview, coupon UI, child deal reconfigure behavior, BOGO, free-item rewards, usage limits, category/subcategory component eligibility, and BundleBuilder/ComboBuilder are not built yet.
...
- Public storefront pages include a temporary subtle `Platform Admin` footer shortcut to `/platform` for development convenience. Remove or protect this when real auth/role enforcement is introduced.
...
- After Specials/Mix-and-Match stabilizes, prioritize operational launch gaps before shiny marketing features: dietary/allergen flags, staff-friendly location controls for Quick 86, realtime staff orders, staff-entered / phone orders, and prominent staff order notes. Customer order status now exists as a safe refresh-based page; realtime/SMS/account lookup remain future work.
- Future customer/marketing items are tracked but deferred: customer accounts/order history/reorder, PWA/installable app support, nutritional info, catering requests, abandoned-cart recovery, gift cards/store credit, age verification if regulated products are supported, and advanced notifications. PWA scope should include customer install for menu/order/status, future reorder/rewards, staff tablet install for orders, future realtime/notifications, manifest/icons/theme color/standalone mode, safe-area handling, browser install testing, fallback behavior, and tenant branding.
- Custom domains are planned/deferred: support `www.restaurant.com`, `order.restaurant.com`, `menu.restaurant.com`, retain MenuPilot fallback URLs such as `/businesses/[businessSlug]/menu`, and later add a `business_domains` table or equivalent with verification status/token, host-based tenant resolution, Vercel custom-domain integration, and SSL/certificate workflow.
- Digital menu boards/signage are planned/deferred and distinct from printable/customer ordering menus. Planned routes include `/businesses/[businessSlug]/boards/[boardSlug]`, `/businesses/[businessSlug]/locations/[locationSlug]/boards/[boardSlug]`, and custom-domain board URLs. Boards should be fullscreen TV layouts with no cart/checkout, large typography, active specials banners, category/product selection, price/image visibility, rotation timing, auto-refresh, future Realtime, and Quick 86 sold-out hiding. Hardware notes: Raspberry Pi OS with Desktop + Chromium kiosk mode, Chromebox/mini PC/Android TV/Fire TV/commercial signage players are reasonable paths; Arduino is not appropriate for browser-rendered boards but may help later with LED signs, buzzers, pickup displays, or buttons.
...
- Checkout total composition is server-authoritative: configured item/deal subtotal, passive Specials discounts, discounted taxable subtotal, service fee, tax, tip, then final total. Tax is calculated after discounts. Tips are only accepted when enabled and currently use discounted subtotal before tax/fees as the basis. Negative tips are normal validation failures and must not create orders.
...
- Tenant admin landing pages should separate Product Catalog, Specials & Promotions, Media Library, Orders & Operations, and Storefront. The Product Catalog workspace owns products, categories/subcategories, reusable variants, and reusable modifiers; product variant and modifier assignments remain product-specific setup.
...
- Product defaults are customer-facing selected state, not magic pricing. Direct Add to Deal should not auto-pick missing required modifiers. Under-selected included allowances should send the customer into the configurator, where they can see the allowance and intentionally add with zero optional selections if the group's min rules allow it.
- Temporary operational availability is not the same as permanent admin enable/disable. Quick 86 rows hide/block products or Modifier Options only while an active business-wide override applies in current public menu/builder/checkout paths. Location-specific override support should take precedence over business-wide overrides when location scope is wired.
...
6. Keep full draft/publish versioning, billing/subscriptions, AI Owner Copilot, multiple builder visual layouts, BundleBuilder/ComboBuilder, printable menu builder, PWA implementation, custom domains, digital menu boards/signage, hardware display support, and customer-facing AI deferred.

...
Tenant context resolves business and location slugs/IDs centrally. Tenant-scoped storefront, menu, specials, checkout, order status, staff orders, business admin, catalog products, catalog modifiers, and media routes now exist. Catalog administration lives only under `/businesses/[businessSlug]/admin/catalog/...`; obsolete unscoped and pre-catalog admin routes have been removed.
...
Default modifier selections are selected modifiers and consume included selection slots. Customer builders hydrate valid product defaults as selected state, so required groups can be satisfied by real defaults. Quick/direct add paths must not invent missing required defaults. If a direct add or Add to Deal path sees an included Modifier Group with remaining unused included selections, it should open the configurator so the customer can use or intentionally skip those choices.
...
Basic checkout tax, service fee, and tip settings are also stored in
`business_pricing_settings`. Checkout totals are composed server-side after
configured-product/deal pricing and passive Specials discounts. Tax and service
fees use the discounted subtotal; tips are only accepted when enabled and use the
discounted subtotal before tax/fees as the current tip basis. Orders snapshot
applied tax, service fee, tip, final total, and the tax/fee/tip settings used.
...
| Public storefront/menu | Tenant route started | `/businesses/[businessSlug]` is the selected business storefront landing page. `/businesses/[businessSlug]/menu` loads the selected business menu and guards nested products/media by business. `/businesses/[businessSlug]/specials` lists active Specials & Deals for the same business, uses build actions for orderable/Mix deals, and keeps passive specials informational until checkout. Legacy `/menu` still loads seeded Pronto Demo via `pronto-demo`. |
...
| Checkout | Tenant route started | Legacy `/checkout` still resolves Pronto Demo/main-street. `/businesses/[businessSlug]/checkout` resolves the business and default location, validates active/orderable status, rejects cross-tenant carts, uses server-side cart validation/repricing, applies passive Specials, then calculates business-level service fee, tax, and optional tip before creating unpaid pickup orders. Successful tenant orders link to `/businesses/[businessSlug]/orders/[orderNumber]`. Transaction/RPC still needed. |
| Staff orders | Tenant route started | Legacy `/staff/orders` still resolves Pronto Demo/main-street. `/businesses/[businessSlug]/locations/[locationSlug]/orders` resolves business/location context, filters reads by `business_id` and `location_id`, verifies order ownership before status updates, and displays Specials subtotal/discount, service fee, tax, tip, total, and applied discount snapshots when present. |
...
| Tenant admin context | Started | `features/tenant` resolves business/location context and `/businesses/[businessSlug]/admin` provides top-level navigation for Product Catalog, Specials & Promotions, Media Library, Orders & Operations, and Storefront Preview. `/businesses/[businessSlug]/admin/catalog` groups products, categories/subcategories, reusable variants, and reusable modifiers. |
| Product admin | Working | `/businesses/[businessSlug]/admin/catalog/products...` supports business-scoped product reads, core product actions, categories, variants, modifier assignments, overrides, and temporary product 86 toggles. |
| Variant admin | Working | Reusable groups/options and per-product overrides |
| Modifier admin | Working | `/businesses/[businessSlug]/admin/catalog/modifiers...` implements Modifier Category -> Modifier Group -> Modifier Option Group -> Modifier Option with tenant-scoped reads/writes and temporary option 86 toggles. |
| Media Library | Working | `/businesses/[businessSlug]/admin/media` lists, uploads, imports, and edits media for the selected business; storage paths use selected `business_id`. |
| Manager workspace | Started | Location-scoped manager dashboard, orders, and temporary product/modifier sold-out controls exist under `/businesses/[businessSlug]/locations/[locationSlug]/manager`. Authentication and manager catalog permissions remain pending. |
| Employee management | Started | One owner page invites employees, saves profile/contact information, assigns business/location and Manager or Staff role, and configures per-location permissions in `employee_permissions`; authenticated enforcement remains pending. |
...
- `/businesses/[businessSlug]/specials` tenant-scoped public Specials & Deals page
- `/checkout` pickup checkout
- `/businesses/[businessSlug]/checkout` tenant-scoped checkout using the business default location
- `/businesses/[businessSlug]/orders/[orderNumber]` customer-safe tenant-scoped order status page
...
- `/businesses/[businessSlug]/admin/catalog/products...` tenant-scoped product administration
- `/businesses/[businessSlug]/admin/catalog/modifiers...` tenant-scoped reusable modifier management
...
- Seeded-demo public `/menu`, `/checkout`, and `/staff/orders` routes remain pending their separate public/staff access reviews. Obsolete admin route families have been removed.
...
- Draft/publish versioning, billing/subscriptions, AI Owner Copilot, multiple builder visual layouts, BundleBuilder/ComboBuilder, printable menu builder, custom domains, PWA/installable app support, and digital menu boards/signage are intentionally deferred.
...
- Product default modifiers hydrate into customer builders and DealBuilder child configurators. DealBuilder quick Add to Deal uses real product defaults only; missing required defaults or under-selected included allowances open ProductConfigurator instead of adding an incomplete child. Once the configurator is open, optional included groups can still be intentionally left empty if their min rules allow it.
- Launch-critical operational gaps now tracked include dietary/allergen flags, realtime staff order updates, staff-entered / phone orders, and more prominent order/customer notes in staff views. Customer-facing order status now exists at `/businesses/[businessSlug]/orders/[orderNumber]` and reflects current saved staff status on refresh, without realtime/SMS/account lookup. Quick 86 / temporarily sold out now has schema, pure resolver, business-wide tenant admin toggles for products and Modifier Options, public menu hiding, builder filtering, DealBuilder/Mix child choice filtering, and checkout rejection for active business-wide overrides. Staff/location-specific controls and realtime customer refresh remain deferred.
- Customer accounts, order history, reorder, PWA installability, nutritional info, catering requests, abandoned-cart recovery, gift cards/store credit, and advanced marketing remain V1/future work and should not block the immediate Specials/Mix-and-Match thread. PWA planning should cover customer menu/order/status install use, future reorder/rewards, staff tablet orders, future realtime/notifications, manifest/icons/theme color/standalone mode, safe-area handling, mobile/desktop browser testing, fallback behavior, and tenant branding.
- Custom domains are planned/deferred. Future support should cover `www.restaurant.com`, `order.restaurant.com`, `menu.restaurant.com`, MenuPilot fallback slug URLs, a `business_domains` table or equivalent, verification status/token, `verified_at`, host-based tenant resolution, Vercel custom-domain integration, and SSL/certificate verification.
- Digital menu boards/signage are planned/deferred and are separate from printable menus and customer ordering menus. Future routes should support business/location board slugs, fullscreen TV layouts, active specials banners, product/category selection, price/image visibility, rotation timing, auto-refresh, future Realtime, Quick 86 hiding, and hardware guidance for Raspberry Pi, Chromebox, mini PC, Android TV, Fire TV, and commercial signage players. Arduino is not appropriate for browser-rendered boards, but may be useful later for hardware extras.
...
- Added tenant resolver helpers and the `/businesses/[businessSlug]/admin` shell. Catalog product operations now live under `/businesses/[businessSlug]/admin/catalog/products...`.
- Added tenant-scoped Modifier Library operations under `/businesses/[businessSlug]/admin/catalog/modifiers...`.
- Added tenant-scoped Media Library operations under `/businesses/[businessSlug]/admin/media`.
- Removed unscoped `/admin/*` and pre-catalog tenant product/modifier route families on 2026-09-06.
...
- Updated ProductConfigurator return mode for orderable deal component pricing context. DealBuilder child customization now shows fixed component price plus extras or included/free plus extras in the builder sticky submit total, hides normal base/variant prices inside fixed/included deal component selectors, keeps upcharges/extras visible, and preserves the normal configured-product pricing snapshot for server-side extra calculation.
...
- Added tenant-scoped public Specials & Deals page at `/businesses/[businessSlug]/specials`. The menu header links to it, menu Current Specials can link to the full page, passive specials show checkout-applied copy, orderable deals use Build Deal, and Mix & Match deals use Build Mix & Match.
- Added a temporary subtle Platform Admin footer shortcut on public storefront surfaces. This is convenience only until auth/role protection exists; Platform Admin remains internal and should not be publicly exposed.
- Added Quick 86 / temporarily sold out foundation. Migration `026_operational_availability.sql` adds product and Modifier Option operational availability override tables with business-wide and future location-specific scope. `features/availability` adds the shared resolver and server actions. Tenant Product Admin and Modifier Library rows can mark products/options temporarily sold out or available again.
- Wired Quick 86 / temporarily sold out into public ordering paths. Public menu reads hide active business-wide sold-out products; ProductConfigurator loading blocks sold-out products and filters sold-out Modifier Options after product/variant overrides are applied; required groups with no available options block add-to-cart; DealBuilder and Mix & Match child product choices exclude active sold-out products; checkout reloads operational availability server-side and rejects stale sold-out products or Modifier Options.
- Added customer-facing order status at `/businesses/[businessSlug]/orders/[orderNumber]`. Tenant checkout confirmation links to the route after successful order creation; the page loads by business slug and order number, shows safe order/status/summary details, nested deal/Mix children, modifiers, discounts, restaurant info, and current saved staff status on refresh without exposing internal IDs, payment data, customer email, or customer phone.

### 2026-06-11

- Added basic business-level checkout tax, service fee, and tip support. Pricing settings now include sales tax percent, service fee type/value, and tips enabled. Checkout recalculates totals server-side after configured item/deal pricing and passive Specials discounts, rejects negative tips, rejects tips when disabled, stores order tax/fee/tip totals and snapshots, and displays service fee/tax/tip rows in checkout, confirmation, customer order status, and staff orders when present.
